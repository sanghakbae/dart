...
// 같은 회사 같은 서식인데 해마다 연결·별도가 뒤집혔다(2024·2023 연결, 2025·2022·2021 별도).
// 원인은 연결 표만 복원에 실패한 해가 남은 별도 표만 보고 '별도' 로 판정된 것이다.
test('연결 표가 복원되지 않아도 제목이 있으면 연결이다', () => {
  const doc = docOf(`
연결 재무상태표
제 78 기 2025.12.31 현재
(연결 표는 서식이 달라 계정이 하나도 잡히지 않았다)
재무상태표
제 78 기 2025.12.31 현재
과 목\t제 78 기\t제 77 기
자산총계\t110,000,000\t95,000,000
...
test('사업보고서 절 제목("1. 연결재무제표")도 근거로 본다', () => {
  const doc = docOf(`
1. 연결재무제표
(표가 이미지로만 들어 있어 아무것도 복원되지 않았다)
3. 재무제표
재무상태표
과 목\t제 15 기\t제 14 기
자산총계\t34,188,231,721\t30,344,985,559
...
// 반대 방향도 지켜야 한다 — 별도 보고서의 주석에는 연결 이야기가 흔히 나온다.
// 그걸 근거로 삼으면 별도 보고서가 연결로 둔갑한다(예전 텍스트 검색이 그랬다).
test('주석의 연결 언급만으로는 연결이 되지 않는다', () => {
  const doc = docOf(`
재무상태표
과 목\t제 15 기\t제 14 기
자산총계\t34,188,231,721\t30,344,985,559
...
주석
1. 회사의 개요
회사는 종속기업이 없어 연결재무제표를 작성하지 않습니다.
연결재무제표에 대한 주석은 해당사항이 없습니다.
`)
  assert.equal(parseStatements(doc, { fiscalYear: 2025 }).basis, '별도')
})
