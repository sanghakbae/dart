...
export function resolvePeriods(blocks, meta) {
  ...
  // 연도 뒤 구분자에 '년' 도 넣어야 한다. 세스코 감사보고서는 기간을
  // "제 50 기 2025년 12월 31일 현재" 로 적는데, 점·하이픈만 받으면 이 형식이
  // 통째로 안 걸려 제N기↔연도 연결이 끊긴다 — 그러면 표지 추정값으로 떨어지고,
  // 그 값이 2024 로 잡혀 2025년 보고서가 2024년 자리를 덮어썼다.
  const re = /제\s*(\d+)\s*(?:\(\s*[당전]\s*\)|[당전])?\s*기(?:말)?\s*(?:\(\s*[당전]\s*\)\s*)?(\d{4})\s*[년.\-/]\s*\d{1,2}/g
